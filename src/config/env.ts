...
export const env = cleanEnv(
  process.env,
  {
    ...
    KAFKA_RETRY_ATTEMPTS: num({ default: 3 }),
    KAFKA_RETRY_DELAY_MS: num({ default: 500 }),
    KAFKA_DEAD_LETTER_TOPIC_SUFFIX: str({ default: '.dlq' }),

    JWT_ACCESS_SECRET: str(),
    JWT_ACCESS_TTL: str({ default: '15m' }),
    JWT_REFRESH_SECRET: str(),
    JWT_REFRESH_TTL: str({ default: '7d' }),
    JWT_ALGORITHM: str({ choices: ['HS256'], default: 'HS256' }),
